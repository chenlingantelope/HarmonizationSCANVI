...
f = open('../PopRemove1/'+plotname+'.acc.res.txt', "w+")
g = open('../PopRemove1/'+plotname+'.res.txt', "w+")
...
for rmCellTypes in dataset2.cell_types[:6]:
    pbmc = deepcopy(dataset2)
    newCellType = [k for i, k in enumerate(dataset2.cell_types) if k not in [rmCellTypes,'Others']]
    pbmc.filter_cell_types(newCellType)
    gene_dataset = GeneExpressionDataset.concat_datasets(dataset1, pbmc)

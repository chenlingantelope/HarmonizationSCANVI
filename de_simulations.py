...
count_matrix = pd.read_csv(os.path.join(save_path, "filtered_DE.obsv.csv"),
                           sep=",", index_col=0).T
label_array = pd.read_csv(os.path.join(save_path, "filtered_DE.cell_meta.csv"),
                          sep=",", index_col=0)["pop"].values
batch_array = pd.read_csv(os.path.join(save_path, "filtered_batchid.csv"),
...
trainer_scanvi = AlternateSemiSupervisedTrainer(scanvi, gene_dataset,
                                                n_epochs_classifier=10, lr_classification=5 * 1e-3, kl=1)
...
for key in theoretical_FC.columns:
    ...
    # TODO: for loop 20 times for randomization
    ...
    pickle.dump(results_DE_scVI_A, open(os.path.join(save_path, "results0.dic"), "wb"))
    pickle.dump(results_DE_scVI_B, open(os.path.join(save_path, "results1.dic"), "wb"))
    pickle.dump(results_DE_scVI_AB, open(os.path.join(save_path, "results2.dic"), "wb"))
    pickle.dump(results_DE_scANVI, open(os.path.join(save_path, "results3.dic"), "wb"))

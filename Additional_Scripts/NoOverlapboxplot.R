...
# files = list.files()
# files = files[grep('res.txt',files,fixed=T)]
# data = lapply(files,function(X){
# 	temp = read.csv(X,header=F,sep='\t',as.is=T)
...
data2 = read.csv('NoOverlap.kp.res.txt',header=F,sep='\t',as.is=T)
...
xlab('Cell Type')
ggsave('all.BE.pdf',height=3.2,width=5)
...
ylab('kNN Purity') + 
xlab('Cell Type')
ggsave('all.acc.pdf',height=3.2,width=5)
